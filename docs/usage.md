...
点击主界面「模型检测」旁的「MCP 管理」入口后，弹出服务开关面板；点击外部或关闭按钮收起。面板读取 `~/.codex/config.toml` 中的 `mcp_servers`，为每个服务提供独立开关。未设置 `enabled` 时按启用显示，关闭写入 `enabled = false`，重新开启写入 `enabled = true`；不删除服务、命令、地址或凭据。保存后请重启 Codex 或相关会话。

修改沿用现有备份、恢复和文件变更检查机制。「恢复上次配置」可撤销最近一次切换。当前沿用主配置读取要求，需先成功读取本工具支持的自定义提供方及 API Key 配置。支持独立服务表及内联表；缺少 `enabled` 的隐式或点分表如无法安全定位，会提示转换为 `[mcp_servers.服务名]` 后再操作，不会覆盖文件。
